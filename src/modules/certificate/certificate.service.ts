...
export class CertificateService {
  ...
  async renderImageFromHTML(
    credentialId: string,
    templateId: string,
    res: Response,
  ): Promise<any> {
    const apiId = 'api.get.Certificate.Image';
    ...
    try {
      ...
      // Set large enough viewport to include full certificate
      await page.setViewport({
        width: 800,
        height: 1200, // Increased height to prevent cut-off
      ...
      // Optional: capture specific element instead of whole page
      const elementHandle = await page.$('body'); // or a class like '.certificate'
      ...
      this.loggerService.error('Error rendering certificate image:', error);
      return APIResponse.error(
        res,
        apiId,
        'Error rendering certificate image',
        'INTERNAL_SERVER_ERROR',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
